            ...
            steps {
                script {
                    ...
                    if (branch.startsWith('lte-')) {
                        ...
                        sh "sshpass -p $OFFLINE_AS_PWD ssh $OFFLINE_AS_USERNAME@$OFFLINE_AS_HOST sh $TO_LTE_CD_PATH/admin/robot.sh '\"$branch\" \"$gitCommitAuthorName\" \"api-hep-admin\" \"LTE环境\" \"$gitCommitMessage\" \"$changes\" \"green\"'"
                        ...
                        sh "sshpass -p $OFFLINE_AS_PWD ssh $OFFLINE_AS_USERNAME@$OFFLINE_AS_HOST sh $TO_DEV_CD_PATH/admin/robot.sh '\"$branch\" \"$gitCommitAuthorName\" \"api-hep-admin\" \"DEV环境\" \"$gitCommitMessage\" \"$changes\" \"green\"'"
                        ...
                        sh "sshpass -p $OFFLINE_AS_USERNAME ssh $OFFLINE_AS_USERNAME@$OFFLINE_AS_HOST sh $TO_UAT_CD_PATH/admin/robot.sh '\"$branch\"' \"$gitCommitAuthorName\" 'api-hep-admin' 'UAT环境' '\"$gitCommitMessage\"' '\"$changes\"' 'green'"
                        ...
                        //sh "sshpass -p $AS_PWD ssh $AS_USERNAME@$AS_HOST sh $TO_PRD_CD_PATH/admin/robot.sh '$branch' '$gitCommitAuthorName' 'api-hep-admin' 'PRD环境发布' '$gitCommitMessage' '$changes' 'green'"
                        //sh "sshpass -p $AS_PWD ssh $AS_USERNAME@$AS_HOST sh $TO_PRD_CD_PATH/admin/robot.sh '\"${branch}\"' '\"${gitCommitAuthorName}\"' 'api-hep-admin' 'PRD环境' 'green' '\"${gitCommitMessage}\"'"
                    }
                }
            }
        }
