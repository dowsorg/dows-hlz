...
public class ExperimentStartDealer extends BaseEventDealer {
    ...
    protected boolean coreDeal(EventDealResult rst, SysEventRow row, SysEventRunStat stat) {
        ...
        if(exptColl.hasSandMode()) {
            ...
            mapTimers.values().forEach(item -> {
                item.setPaused(true)
                        .setPauseTime(now)
                        .setPauseCount(Optional.ofNullable(item.getPauseCount()).orElse(0) + 1)
                        .setPeriodTimer(0L)
                        .setState(EnumExperimentState.PREPARE.getState());
            });
        ...
        // 保存或更新实验计时器
        if(!experimentTimerBiz.saveOrUpdateExperimentTimeExperimentState(experimentInstanceId,rowsTimer, EnumExperimentState.PREPARE)){
