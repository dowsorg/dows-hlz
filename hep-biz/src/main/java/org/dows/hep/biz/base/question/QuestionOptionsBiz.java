...
public class QuestionOptionsBiz {
    ...
    public Boolean saveOrUpdBatch(List<QuestionOptionsEntity> optionList) {
        ...
        // check
        List<String> idList = optionList.stream()
                .map(QuestionOptionsEntity::getQuestionOptionsId)
                .toList();
        ...
        if (optionsEntityList == null || optionsEntityList.isEmpty()) {
            return questionOptionsService.saveBatch(optionList);

        }

        // 新增更新都有的
        Map<String, Long> collect = optionsEntityList.stream()
                .collect(Collectors.toMap(QuestionOptionsEntity::getQuestionOptionsId, QuestionOptionsEntity::getId, (v1, v2) -> v1));
        List<QuestionOptionsEntity> entityList = optionList.stream()
                .peek(option -> {
                    String questionOptionsId = option.getQuestionOptionsId();
                    Long id = collect.get(questionOptionsId);
                    option.setId(id);
                })
                .toList();

        return questionOptionsService.saveOrUpdateBatch(entityList);
//
//        // 新增
