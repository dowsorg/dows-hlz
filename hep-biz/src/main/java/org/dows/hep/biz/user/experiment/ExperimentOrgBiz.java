...
public class ExperimentOrgBiz {
    ...
    public OrgNoticeResponse coreSaveOrgNoticeAction(SaveNoticeActionRequest saveNoticeAction, HttpServletRequest request)  {
        AssertUtil.trueThenThrow(saveNoticeAction==null||saveNoticeAction.getActions().isEmpty()).throwMessage("请选择事件处理措施");
...
/*
        ...
        List<AccountUserResponse> usersByAccountIds = accountUserApi.getUsersByAccountIds(accountIds);
        // 一个账号对应一个用户
        Map<String, String> accountUserIdMapping = usersByAccountIds.stream()
        ...
        Map<String, UserInstanceResponse> userInstanceResponseMap = new HashMap<>();

        accountUserIdMapping.forEach((k, v) -> {
            UserInstanceResponse userInstanceResponse = collect.get(accountUserIdMapping.get(k));
            userInstanceResponseMap.put(k, userInstanceResponse);
        });
