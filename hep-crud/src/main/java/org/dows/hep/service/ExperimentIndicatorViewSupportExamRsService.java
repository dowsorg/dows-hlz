package org.dows.hep.service;

import org.dows.framework.crud.mybatis.MybatisCrudService;
import org.dows.hep.entity.ExperimentIndicatorViewSupportExamRsEntity;
import org.dows.hep.entity.IndicatorViewSupportExamEntity;


/**
 * 查看指标辅助检查类(IndicatorViewSupportExam)表服务接口
 *
 * @author lait
 * @since 2023-04-28 10:30:48
 */
public interface ExperimentIndicatorViewSupportExamRsService extends MybatisCrudService<ExperimentIndicatorViewSupportExamRsEntity> {

}

