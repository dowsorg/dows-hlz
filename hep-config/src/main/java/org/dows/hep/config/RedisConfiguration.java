...
public class RedisConfiguration {
    ...
    @Value("#{${cache.ttl:{T(java.util.Collections).emptyMap()}}}")
    ...
    public CacheManager cacheManager(RedisConnectionFactory redisConnectionFactory) {
        ...
        redisCacheConfiguration = redisCacheConfiguration
                ...
                // 配置缓存空间名称的前缀
                .prefixCacheNameWith("polaris:demo:")
                //全局配置缓存过期时间【可以不配置】
        ...
        for (Map.Entry<String, Long> entry : entries) {
            //指定特定key的过期时间
            //map.put("userCache", redisCacheConfiguration.entryTtl(Duration.ofDays(30L))); // 手动直接设置
            map.put(entry.getKey(), redisCacheConfiguration.entryTtl(Duration.ofSeconds(entry.getValue()))); // 读取配置文件进行设置
        }
